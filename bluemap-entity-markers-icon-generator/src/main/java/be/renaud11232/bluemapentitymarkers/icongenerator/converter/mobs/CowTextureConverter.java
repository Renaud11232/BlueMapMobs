...
public class CowTextureConverter extends TextureConverter {
    public CowTextureConverter() {
        super();
        ...
        registerTextureConversion((texture, icon) -> {
            BufferedImage face = texture.getSubimage(6, 6, 8, 8);
            BufferedImage nose = texture.getSubimage(10, 34, 6, 3);
            icon.drawImage(face, 0, 0, face.getWidth() * 4, face.getHeight() * 4, null);
            icon.drawImage(nose, 4, 20, nose.getWidth() * 4, nose.getHeight() * 4, null);
        });
        registerTextureConversion((texture, icon) -> {
    ...
    protected List<Map.Entry<List<Pattern>, List<String>>> getConversions() {
        return List.of(
                Map.entry(
                        List.of(Pattern.compile("^cow_(temperate|warm)\\.png$")),
                        List.of("cow_$1.png")
                ),
                Map.entry(
                        List.of(Pattern.compile("^cow_cold\\.png$")),
                        List.of("cow_cold.png")
                ),
                Map.entry(
                        List.of(Pattern.compile("^cow_(cold|temperate|warm)_baby\\.png$")),
                        List.of("cow_$1_baby.png")
                )
        );
    }
}
